...
app.get('/authorize', function(req, res) {
  ...
  res.redirect(encodeURI(`https://data.world/oauth/authorize?client_id=${config.client_id}&redirect_uri=${config.redirect_uri}&response_type=code&code_challenge_method=S256&code_challenge=${config.code_challenge}`))
});

app.get('/', function (req, res) {
  var code = req.query.code;
  if (code){
    axios.post(encodeURI(`https://data.world/oauth/access_token?code=${code}&client_id=${config.client_id}&client_secret=${process.env.DATAWORLD_SECRET}&grant_type=authorization_code&code_verifier=${config.code_verifier}`)).then((response) =>{
